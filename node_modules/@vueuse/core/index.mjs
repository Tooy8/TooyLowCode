...
function useFetch(url, ...args) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    const context = { url: resolveUnref(url), options: __spreadValues$b(__spreadValues$b({}, defaultFetchOptions), fetchOptions), cancel: () => {
      isCanceled = true;
    } };
...
const useImage = (options, asyncStateOptions = {}) => {
  const state = useAsyncState(() => loadImage(resolveUnref(options)), void 0, __spreadValues$8({
    resetOnExecute: true
  }, asyncStateOptions));
...
function useShare(shareOptions = {}, options = {}) {
  ...
  const share = async (overrideOptions = {}) => {
    if (isSupported.value) {
      const data = __spreadValues$3(__spreadValues$3({}, resolveUnref(shareOptions)), resolveUnref(overrideOptions));
      let granted = true;
