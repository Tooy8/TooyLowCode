...
function onKeyDown(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps$8(__spreadValues$k({}, options), { eventName: "keydown" }));
}
function onKeyPressed(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps$8(__spreadValues$k({}, options), { eventName: "keypress" }));
}
function onKeyUp(key, handler, options = {}) {
  return onKeyStroke(key, handler, __spreadProps$8(__spreadValues$k({}, options), { eventName: "keyup" }));
}
    ...
    try {
      ...
      if (rawValue == null) {
        ...
        if (shared.isFunction(mergeDefaults))
          ...
          return __spreadValues$i(__spreadValues$i({}, rawInit), value);
        return value;
...
function chainCallbacks(...callbacks) {
  return (ctx) => {
    callbacks.forEach(async (callback) => {
      if (callback)
        ctx = __spreadValues$b(__spreadValues$b({}, ctx), await callback(ctx));
    });
    return ctx;
  };
}
  ...
  function useFactoryFetch(url, ...args) {
    ...
    if (args.length > 0) {
      if (isFetchOptions(args[0])) {
        options = __spreadProps$3(__spreadValues$b(__spreadValues$b({}, options), args[0]), {
    ...
    if (args.length > 1 && isFetchOptions(args[1])) {
      options = __spreadProps$3(__spreadValues$b(__spreadValues$b({}, options), args[1]), {
...
function useFetch(url, ...args) {
  ...
  const execute = async (throwOnFailed = false) => {
    ...
    const context = { url: shared.resolveUnref(url), options: __spreadValues$b(__spreadValues$b({}, defaultFetchOptions), fetchOptions), cancel: () => {
      isCanceled = true;
    } };
...
const useImage = (options, asyncStateOptions = {}) => {
  const state = useAsyncState(() => loadImage(shared.resolveUnref(options)), void 0, __spreadValues$8({
    resetOnExecute: true
  }, asyncStateOptions));
...
function useShare(shareOptions = {}, options = {}) {
  ...
  const share = async (overrideOptions = {}) => {
    if (isSupported.value) {
      const data = __spreadValues$3(__spreadValues$3({}, shared.resolveUnref(shareOptions)), shared.resolveUnref(overrideOptions));
      let granted = true;
