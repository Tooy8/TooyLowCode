...
function useGlobalConfig(globalConfig, localProps = []) {
...
function conclude(configSequence, config = {}) {
  let {
    name = "",
    type,
  ...
  if (defaultIsDynamic) {
    return conclude(configSequence, __spreadProps(__spreadValues({}, config), {
...
function objectFilter(obj, predicate) {
  return Object.keys(obj).filter((key) => predicate(key)).reduce((prev, curr) => {
    prev[curr] = obj[curr];
    return prev;
  }, {});
}
function getLocalListeners(listeners) {
  return objectFilter(listeners, (key) => !key.startsWith("hook:"));
}
function listenGlobalHooks(globalHooks) {
  if (notEmpty(globalHooks)) {
    ...
    this.$emit = function() {
      ...
      (_a = globalHooks[eventName]) == null ? void 0 : _a.apply(this, args);
    };
  }
}
export { conclude, getLocalListeners, listenGlobalHooks, useGlobalConfig };
